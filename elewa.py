...
def add_user(username, password, is_admin=False):
    conn = sqlite3.connect("users.db")
    c = conn.cursor()
    hashed_pass = bcrypt.hash(password)  # ✅ securely hash the password
    try:
...
def login_user(username, password):
    ...
    conn.close()
    if result:
        stored_hashed_pass, is_admin = result
        if bcrypt.verify(password, stored_hashed_pass):  # ✅ verify password
